...
public class EadoreChunkGenerator extends NoiseChunkGenerator<EadoreGenSettings>
{
    ...
    public EadoreChunkGenerator(IWorld worldIn, BiomeProvider provider, EadoreGenSettings settingsIn)
    {
    ...
    private double getNoiseDepthAt(int noiseX, int noiseZ)
    {
        ...
        if (d0 < 0.0D) d0 = -d0 * 0.3D;

        d0 = d0 * 3.0D - 2.0D;
        if (d0 < 0.0D) d0 = d0 / 28.0D;
        else {
            if (d0 > 1.0D) d0 = 1.0D;
